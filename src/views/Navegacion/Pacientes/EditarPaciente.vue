...
const datosPaciente = async () => {
  ...
  if (response.sexo === 'Mujer') {
    sexo.value = false
  } else {
    sexo.value = true
  }
  model.nombre = response.nombre
  model.apellido = response.apellido
  model.edad = response.fechaNacimiento.substring(0, 10)
  model.ocupacion = response.ocupacion
  model.telefono = response.telefono
  model.institucion = response.institucion
  model.domicilio = response.domicilio
  model.codigoPostal = response.codigoPostal
  model.fisioterapeuta = response.fisioterapeutaId
  model.estadoCivil = response.estadoCivilId
  model.tipoPaciente = response.tipoPago
  imageUrl.value = base64IMG + response.fotoPerfil
...
const agregarPaciente = async () => {
  ...
  if ($v.value.$invalid) {
    spinner.value = false
    NotificacionesModal.PantallaError('Verifica tus datos')
    return
  }

  let response = await pacientesCommand.actualizarDataPaciente(
    pacienteId.value,
    model.nombre,
    model.apellido,
    model.institucion,
  ...
  <div class="flex telefono:flex-col">
    ...
    <section class="desktop:w-9/12 laptop:w-9/12 tablet:w-full telefono:w-full">
      <details open class="cursor-pointer">
        <summary class="text-principal text-xl mb-2">Información general</summary>
        <hr class="border-b-blue-500 mb-3" />
        <div class="grid gap-6 mb-6 md:grid-cols-2">
          <div>
            <label class="block mb-2 text-sm font-medium text-gray-600"
              >Nombre(s) <span class="text-blue-600">*</span>
            </label>
            <input
              type="text"
              class="input-primary"
              ...
              required
            />
            <span v-if="$v.nombre.$error" class="text-red-500 text-xs"
              >El nombre es obligatorio</span
            >
          </div>
          <div>
            <label class="block mb-2 text-sm font-medium text-gray-600"
              >Apellidos <span class="text-blue-600">*</span></label
            >
            <input
              type="text"
              class="input-primary"
              ...
              required
            />
            <span v-if="$v.apellido.$error" class="text-red-500 text-xs"
          ...
          <div>
            <label class="block mb-2 text-sm font-medium text-gray-600"
              >Fecha de nacimiento <span class="text-blue-600">*</span>
            </label>
            <input
              type="date"
              class="input-primary"
          ...
          <div>
            <label class="block mb-2 text-sm font-medium text-gray-600"
              >Sexo <span class="text-blue-600">*</span></label
            >
          ...
          <div>
            <label class="block mb-2 text-sm font-medium text-gray-600"
              >Estado civil <span class="text-blue-600">*</span>
            </label>
            ...
            <span v-if="$v.estadoCivil.$error" class="text-red-500 text-xs"
              >El estado civil es obligatorio</span
            >
          </div>
          <div>
            <label class="block mb-2 text-sm font-medium text-gray-600"
              >Ocupación<span class="text-blue-600">*</span></label
            >
            <input
              type="text"
              class="input-primary"
              ...
              required
            />
            <span v-if="$v.ocupacion.$error" class="text-red-500 text-xs"
              >La ocupacion es obligatoria</span
            >
          </div>
          <div>
            <label class="block mb-2 text-sm font-medium text-gray-600"
              >Teléfono<span class="text-blue-600">*</span></label
            >
            <input
              type="tel"
              class="input-primary"
              ...
              required
            />
            <span v-if="$v.telefono.$error" class="text-red-500 text-xs"
              >El telefono debe contener 10 digitos del 0 al 9</span
            >
            <span v-if="verificarTelefono" class="text-red-500 text-xs"
              >El telefono ya esta en uso</span
            >
          </div>

          <div>
            <label class="block mb-2 text-sm font-medium text-gray-600"
              >Institución <span class="text-blue-600">*</span></label
            >
            <input
              type="text"
              class="input-primary"
              ...
              required
            />
            <span v-if="$v.institucion.$error" class="text-red-500 text-xs"
              >La institucion es obligatoria</span
            >
          </div>
          <div>
            <label class="block mb-2 text-sm font-medium text-gray-600"
              >Fisioterapeuta <span class="text-blue-600">*</span></label
            >
            ...
            <span v-if="$v.fisioterapeuta.$error" class="text-red-500 text-xs"
              >El fisioterapeuta es obligatorio</span
            >
          </div>
          <div>
            <label class="block mb-2 text-sm font-medium text-gray-600"
              >Tipo de pago <span class="text-blue-600">*</span></label
            >
            ...
            <span v-if="$v.tipoPaciente.$error" class="text-red-500 text-xs"
              >El tipo de pago es obligatorio</span
            >
          </div>
        </div>
      </details>
      <details open class="cursor-pointer">
        <summary class="text-principal text-xl mb-2">Información de residencia</summary>
        <hr class="mb-3" />
        <div class="flex gap-6 telefono:flex-wrap telefono:gap-1">
          <div class="mb-6 basis-3/4 telefono:basis-full">
            <label class="block mb-2 text-sm font-medium text-gray-600"
              >Domicilio <span class="text-blue-600">*</span></label
            >
            <input
              type="text"
              class="input-primary"
              ...
              required
            />
            <span v-if="$v.domicilio.$error" class="text-red-500 text-xs"
              >El domicilo es obligatorio</span
            >
          </div>
          <div class="mb-6 basis-1/4 telefono:basis-full">
            <label class="block mb-2 text-sm font-medium text-gray-600"
              >Código Postal <span class="text-blue-600">*</span></label
            >
            <input
              type="text"
              class="input-primary"
              placeholder="00030"
              v-model="model.codigoPostal"
              maxlength="5"
              required
            />
            <span v-if="$v.codigoPostal.$error" class="text-red-500 text-xs"
              >El codigo postal es obligatorio y debe tener 5 digitos</span
            >
          </div>
        </div>
      </details>
      <div class="flex justify-end gap-6 flex-wrap telefono:flex-col-reverse">
        ...
        <button
          ...
          :disabled="spinner"
          :class="{ 'bg-[#238dff]': spinner }"
          class="button-primary"
        >
          <div v-if="spinner === false">Guardar</div>
