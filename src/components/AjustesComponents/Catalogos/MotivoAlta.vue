...
onMounted(() => {
    getMotivos(filtro.value)
})

watch(() => filtro.value, (newFiltro) => {
    filtro.value = newFiltro
    getMotivos(newFiltro)
})

const modificar = (motivo) => {
    motivo.editar = !motivo.editar
}

const guardar = async (motivo, id, descripcion, status) => {
    motivo.guardando = true
    // Aquí puedes agregar cualquier lógica adicional para guardar los cambios
    let response = await catalogosCommand.editarMotivoAlta(id, descripcion, status)
    if (response === true){
        motivo.editar = false
        getMotivos(filtro.value)
    }
...
const getMotivos = async (boolean) => {
    motivos.value = await catalogosQueries.getMotivAlta(boolean)
    motivos.value.reverse()
    // Añadir la propiedad `editar` a cada motivo
    motivos.value.forEach(e => {
        e.editar = false
        e.guardando = false
    })
}
...
const filtros = () => {
    getMotivos(filtro.value = !filtro.value)
}
const nuevoCat = async () => {
    spinner.value = true
    await catalogosCommand.agregarMotivoAlta(nuevoCatalogo.value)
    nuevoCatalogo.value = ''
    nuevo.value = false
    getMotivos(filtro.value)
    spinner.value = false
    ...
    <div class="overflow-x-auto style_scroll">
        <div class="text-gray-600 mb-1 flex justify-between bg-white items-center">
            <div class="flex items-center gap-2">
                Motivos de alta
                <div title="Contacte con el equipo de desarrollo para modificar este catalogo">
        ...
        <div class="max-h-96 overflow-y-auto style_scroll">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tr class="bg-white border-b hover:bg-gray-50 cursor-pointer group"
                    v-for="motivo in motivos" :key="motivo.id">
                    <th :class="{'animate-pulse': motivo.guardando}" scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        <input type="text" v-model="motivo.descripcion" :disabled="!motivo.editar"
                               :class="{'bg-white border rounded-[3px] border-gray-300 p-0': motivo.editar, 'border-none group-hover:bg-gray-50 p-0': !motivo.editar}">
                        <span v-if="motivo.status" title="Activo" class="text-green-500 ml-2">Activo</span>
                        <span v-else title="Inactivo" class="text-yellow-500 ml-2">Inactivo</span>
                    </th>
                    <td class="px-6 py-4 flex gap-2">
                        <svg class="hover:stroke-blue-500 cursor-pointer" @click="modificar(motivo)"
                        ...
                        <div class="w-[16px] h-[18px] grid-center hover:text-yellow-500" @click="guardar(motivo, motivo.motivoAltaId,motivo.descripcion, !motivo.status)">
                            <svg
                        ...
                        <svg v-show="motivo.editar"
                             @click="guardar(motivo, motivo.motivoAltaId,motivo.descripcion, motivo.status)"
                             xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
