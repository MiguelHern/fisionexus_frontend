...
onMounted(() => {
    getServicios(filtro.value)
})

watch(() => filtro.value, (newFiltro) => {
    filtro.value = newFiltro
    getServicios(newFiltro)
})

const modificar = (servicio) => {
    servicio.editar = !servicio.editar
}

const guardar = async (servicio, id, descripcion, status) => {
    servicio.guardando = true
    // Aquí puedes agregar cualquier lógica adicional para guardar los cambios
    let response = await catalogosCommand.editarServicios(id, descripcion, status)
    if (response === true){
        servicio.editar = false
        getServicios(filtro.value)
    }
    else
        servicio.guardando = true
...
const getServicios = async (boolean) => {
    servicios.value = await catalogosQueries.getServicios(boolean)
    servicios.value.reverse()
    // Añadir la propiedad `editar` a cada servicio
    servicios.value.forEach(e => {
        e.editar = false
        e.guardando = false
    })
}
...
const filtros = () => {
    getServicios(filtro.value = !filtro.value)
}
const nuevoCat = async () => {
    spinner.value = true
    await catalogosCommand.agregarServicios(nuevoCatalogo.value)
    nuevoCatalogo.value = ''
    nuevo.value = false
    getServicios(filtro.value)
    spinner.value = false
        ...
        <div class="max-h-96 overflow-y-auto style_scroll">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tr class="bg-white border-b hover:bg-gray-50 cursor-pointer group"
                    ...
                    <th :class="{'animate-pulse': servicio.guardando}" scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        <input type="text" v-model="servicio.descripcion" :disabled="!servicio.editar"
                               :class="{'bg-white border rounded-[3px] border-gray-300 p-0': servicio.editar, 'border-none group-hover:bg-gray-50 p-0': !servicio.editar}">
                        <span v-if="servicio.status" title="Activo" class="text-green-500 ml-2">Activo</span>
                        <span v-else title="Inactivo" class="text-yellow-500 ml-2">Inactivo</span>
                    </th>
                    <td class="px-6 py-4 flex gap-2">
                        <svg class="hover:stroke-blue-500 cursor-pointer" @click="modificar(servicio)"
                        ...
                        <div class="w-[16px] h-[18px] grid-center hover:text-yellow-500" @click="guardar(servicio, servicio.servicioId,servicio.descripcion, !servicio.status)" >
                            <svg
                        ...
                        <svg v-show="servicio.editar"
                             @click="guardar(servicio, servicio.servicioId,servicio.descripcion, servicio.status)"
                             xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
