...
const modificar = (flujo) => {
    flujo.editar = !flujo.editar
}

const guardar = async (flujo, id, descripcion, status) => {
    flujo.guardando = true
    // Aquí puedes agregar cualquier lógica adicional para guardar los cambios
    let response = await catalogosCommand.editarFlujoVaginal(id, descripcion, status)
    if (response === true) {
        flujo.editar = false
        getFlujos(filtro.value)
    } else
...
const getFlujos = async (boolean) => {
    flujos.value = await catalogosQueries.getFlujo(boolean)
    flujos.value.reverse()
    // Añadir la propiedad `editar` a cada flujo
    flujos.value.forEach(e => {
        e.editar = false
        e.guardando = false
    })
}
...
const filtros = () => {
    getFlujos(filtro.value = !filtro.value)
}
const nuevoCat = async () => {
    spinner.value = true
    await catalogosCommand.agregarFlujoVaginal(nuevoCatalogo.value)
    nuevoCatalogo.value = ''
    nuevo.value = false
    getFlujos(filtro.value)
    spinner.value = false
        ...
        <div class="max-h-96 overflow-y-auto style_scroll">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tr class="bg-white border-b hover:bg-gray-50 cursor-pointer group"
                    v-for="flujo in flujos" :key="flujo.id">
                    <th :class="{'animate-pulse': flujo.guardando}" scope="row"
                        class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        <input type="text" v-model="flujo.descripcion" :disabled="!flujo.editar"
                               :class="{'bg-white border rounded-[3px] border-gray-300 p-0': flujo.editar, 'border-none group-hover:bg-gray-50 p-0': !flujo.editar}">
                        <span v-if="flujo.status" title="Activo" class="text-green-500 ml-2">Activo</span>
                        <span v-else title="Inactivo" class="text-yellow-500 ml-2">Inactivo</span>
                    </th>
                    <td class="px-6 py-4 flex gap-2">
                        <svg class="hover:stroke-blue-500 cursor-pointer" @click="modificar(flujo)"
                        ...
                        <div class="w-[16px] h-[18px] grid-center hover:text-yellow-500" @click="guardar(flujo, flujo.flujoVaginalId,flujo.descripcion, !flujo.status)">
                            <svg
                        ...
                        <svg v-show="flujo.editar"
                             @click="guardar(flujo, flujo.flujoVaginalId,flujo.descripcion, flujo.status)"
                             xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
