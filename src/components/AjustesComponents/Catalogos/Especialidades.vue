...
const modificar = (especialidad) => {
    especialidad.editar = !especialidad.editar
}

const guardar = async (especialidad, id, descripcion, status) => {
    especialidad.guardando = true
    // Aquí puedes agregar cualquier lógica adicional para guardar los cambios
    let response = await catalogosCommand.editarEspecialidad(id, descripcion, status)
    if (response === true) {
        especialidad.editar = false
        getEsp(filtro.value)
    } else
        especialidad.guardando = true
    getEsp(filtro.value)
...
const getEsp = async (boolean) => {
    especialidades.value = await catalogosQueries.getEspecialidad(boolean)
    especialidades.value.reverse()
    // Añadir la propiedad `editar` a cada especialidad
    especialidades.value.forEach(e => {
        e.editar = false
        e.guardando = false
    })
}
...
const filtros = () => {
    getEsp(filtro.value = !filtro.value)
}
const nuevoCat = async () => {
    spinner.value = true
    await catalogosCommand.agregarEspecialidad(nuevoCatalogo.value)
    nuevoCatalogo.value = ''
    nuevo.value = false
    getEsp(filtro.value)
    spinner.value = false
        ...
        <div class="max-h-96 overflow-y-auto style_scroll">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tr class="bg-white border-b hover:bg-gray-50 cursor-pointer group"
                    v-for="especialidad in especialidades" :key="especialidad.id">
                    <th :class="{'animate-pulse': especialidad.guardando}" scope="row"
                        class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        <input type="text" v-model="especialidad.descripcion" :disabled="!especialidad.editar"
                               :class="{'bg-white border rounded-[3px] border-gray-300 p-0': especialidad.editar, 'border-none group-hover:bg-gray-50 p-0': !especialidad.editar}">
                        <span v-if="especialidad.status" title="Activo" class="text-green-500 ml-2">Activo</span>
                        <span v-else title="Inactivo" class="text-yellow-500 ml-2">Inactivo</span>
                    </th>
                    <td class="px-6 py-4 flex gap-2">
                        <svg class="hover:stroke-blue-500 cursor-pointer" @click="modificar(especialidad)"
                        ...
                        <div class="w-[16px] h-[18px] grid-center hover:text-yellow-500" @click="guardar(especialidad, especialidad.especialidadId,especialidad.descripcion, !especialidad.status)">
                            <svg
                        ...
                        <svg v-show="especialidad.editar"
                             @click="guardar(especialidad, especialidad.especialidadId,especialidad.descripcion, especialidad.status)"
                             xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
