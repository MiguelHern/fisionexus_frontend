...
onMounted(() => {
    getEstCivil(filtro.value)
})

watch(() => filtro.value, (newFiltro) => {
    filtro.value = newFiltro
    getEstCivil(newFiltro)
})

const modificar = (estCivil) => {
    estCivil.editar = !estCivil.editar
}

const guardar = async (estCivil, id, descripcion, status) => {
    estCivil.guardando = true
    // Aquí puedes agregar cualquier lógica adicional para guardar los cambios
    let response = await catalogosCommand.editarEstadoCivil(id, descripcion, status)
    if (response === true){
        estCivil.editar = false
        getEstCivil(filtro.value)
    }
    else
        estCivil.guardando = true
...
const getEstCivil = async (boolean) => {
    estadosCivil.value = await catalogosQueries.getEstadoCivil(boolean)
    estadosCivil.value.reverse()
    // Añadir la propiedad `editar` a cada estCivil
    estadosCivil.value.forEach(e => {
        e.editar = false
        e.guardando = false
    })
}
...
const filtros = () => {
    getEstCivil(filtro.value = !filtro.value)
}
const nuevoCat = async () => {
    spinner.value = true
    await catalogosCommand.agregarEstadoCivil(nuevoCatalogo.value)
    nuevoCatalogo.value = ''
    nuevo.value = false
    spinner.value = false
    getEstCivil(filtro.value)
        ...
        <div class="max-h-96 overflow-y-auto style_scroll">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tr class="bg-white border-b hover:bg-gray-50 cursor-pointer group"
                    v-for="estCivil in estadosCivil" :key="estCivil.id">
                    <th :class="{'animate-pulse': estCivil.guardando}" scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        <input type="text" v-model="estCivil.descripcion" :disabled="!estCivil.editar"
                               :class="{'bg-white border rounded-[3px] border-gray-300 p-0': estCivil.editar, 'border-none group-hover:bg-gray-50 p-0': !estCivil.editar}">
                        <span v-if="estCivil.status" title="Activo" class="text-green-500 ml-2">Activo</span>
                        <span v-else title="Inactivo" class="text-yellow-500 ml-2">Inactivo</span>
                    </th>
                    <td class="px-6 py-4 flex gap-2">
                        <svg class="hover:stroke-blue-500 cursor-pointer" @click="modificar(estCivil)"
                        ...
                        <div class="w-[16px] h-[18px] grid-center hover:text-yellow-500" @click="guardar(estCivil, estCivil.estadoCivilId,estCivil.descripcion, !estCivil.status)">
                        <svg
                             xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                             stroke="currentColor" class="size-4 hover:text-yellow-500">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                  d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99" />
                        </svg>
                        </div>
                        <svg v-show="estCivil.editar"
                             @click="guardar(estCivil, estCivil.estadoCivilId,estCivil.descripcion, estCivil.status)"
                             xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
